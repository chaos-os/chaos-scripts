...
while true do
    ...
    elseif N >= "85" and M == "Charging" then
        os_capture(
            "dunstify 'battery reached limit' 'please plug out your charger' --urgency=low --icon=/usr/share/icons/Numix/16/status/battery-100.svg"
        )
    end
    os.execute(string.format("sleep %s", config.bat_notify_timer))
end
